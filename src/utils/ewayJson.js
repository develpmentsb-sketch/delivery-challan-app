...
// Sub types offered on the portal for an Outward e-way bill (codes from the tool's "Master Codes" sheet)
export const SUB_TYPES = [
  { label: 'Supply', code: 1 },
  { label: 'Export', code: 3 },
  { label: 'Job Work', code: 4 },
  { label: 'For Own Use', code: 5 },
  { label: 'Exhibition or Fairs', code: 12 },
  { label: 'Line Sales', code: 10 },
  { label: 'Recipient Not Known', code: 11 },
  { label: 'SKD/CKD/Lots', code: 9 },
  { label: 'Others', code: 8 }
]
 ...
 *   subType     - optional label from SUB_TYPES (default: Job Work for Job Work challans, otherwise Others)
 *   subTypeDesc - text for the portal's "Specify" box when the sub type is Others (max 20 characters)
 */
export function buildEwayBill({ company, header, billTo, shipTo, rows, totals, eway, subType, subTypeDesc }) {
  ...
  const chosen =
    SUB_TYPES.find((t) => t.label === subType) ||
    (docType === 'Job Work' ? SUB_TYPES.find((t) => t.label === 'Job Work') : SUB_TYPES.find((t) => t.label === 'Others'))
  const subDesc =
    chosen.label === 'Others' ? clean(subTypeDesc || header.purpose || docType).slice(0, 20) : ''
